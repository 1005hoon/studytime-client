...
const EventDetailContainer: React.FC<EventDetailContainerProps> = (props) => {
  return (
    <InputForm width='600px'>
      ...
      <InputForm.Group>
        <h3>
          생성일: {new Date(props.detail.createdAt).toLocaleDateString()}{' '}
        </h3>
      </InputForm.Group>
      <InputForm.Group>
        <label htmlFor=''>유형</label>
        <input type='text' value={props.detail.type as string} />
      </InputForm.Group>
      {props.detail.urlButtonName1 && (
        <InputForm.Group>
          <label htmlFor=''>첫번째 CTA 버튼</label>
          <input type='text' value={props.detail.urlButtonName1 as string} />
          <label htmlFor=''>버튼링크</label>
          <input type='text' value={props.detail.url1 as string} />
        </InputForm.Group>
      )}
      {props.detail.urlButtonName2 && (
        <InputForm.Group>
          <label htmlFor=''>두번째 CTA 버튼</label>
          <input type='text' value={props.detail.urlButtonName2 as string} />
          <label htmlFor=''>버튼링크</label>
          <input type='text' value={props.detail.url2 as string} />
        </InputForm.Group>
      )}
      {(props.detail.imgUrl as string) && (
        <InputForm.Group>
          <label>등록된 미리보기</label>
          <img
